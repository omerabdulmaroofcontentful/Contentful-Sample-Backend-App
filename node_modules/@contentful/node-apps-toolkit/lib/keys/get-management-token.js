...
exports.getManagementToken = exports.createGetManagementToken = void 0;
 ...
 * @internal
 */
const generateOneTimeToken = (privateKey, { appId, keyId }, { log }) => {
    ...
    try {
        ...
        const token = jsonwebtoken_1.sign({}, privateKey, signOptions);
...
const getTokenFromOneTimeToken = async (appToken, { appInstallationId, spaceId, environmentId, }, { log, http }) => {
    const validateStatusCode = utils_1.createValidateStatusCode([201]);
    log(`Requesting CMA Token with given App Token`);
    const response = await http.post(`spaces/${spaceId}/environments/${environmentId}/app_installations/${appInstallationId}/access_tokens`, {
        headers: {
    ...
    log(`Successfully retrieved CMA Token for app ${appInstallationId} in space ${spaceId} and environment ${environmentId}`);
...
exports.createGetManagementToken = (log, http, existingCache) => {
    ...
    return async (privateKey, opts) => {
        ...
        const appToken = generateOneTimeToken(privateKey, { appId: opts.appInstallationId, keyId: opts.keyId }, { log });
        const ott = await getTokenFromOneTimeToken(appToken, opts, { log, http });
        if (opts.reuseToken) {
            const decoded = jsonwebtoken_1.decode(ott);
